...
if st.button("Open dialog", key="first_dialog_button"):
    dialog.open()def dialog_close_button_clicked():
    st.write("###  Dialog close callback results:")
    st.write("#### No preferences saved!")
    dialog.close()
...
if st.button("Open dialog", key="first_dialog_button"):
    dialog.open()    "primeiro_popup", title="Crie sua conta!",
    on_close_button_clicked=botao_fechar_popup, can_be_closed=True)
